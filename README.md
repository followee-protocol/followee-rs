...
**Milestone 3 (single relay) reviewed at `milestone-3-v0.8.1-reviewed`; the bounded v0.9 relay-maintenance pass is delivered and under review at its maintenance gate.** Specification v0.9 is relay-only: it requires update-number assignment, accepted-state commitment, and `v1/changes` visibility to form one observable order so that no successful cursor can overtake a concurrent update that may later become visible. The audit of the Milestone 3 relay found the invariant already structurally satisfied — every store access serializes through one relay-level lock, `commit_current` is the sole update-number assigner, and allocation and commit are one atomic operation in both backends. Review then corrected the lock boundary: publication is now explicitly two-phase, with every bounded state-independent step (size bounds, deterministic CBOR, schema, descriptor binding, strict signature verification, future-bound classification) running *outside* the store lock into an unfabricable prepared-candidate value, while the lock covers only the state-dependent critical section (sticky recheck, current-state comparison, atomic update-number allocation-and-commit, `v1/changes` eligibility) — honouring the v0.9 section 13.2 SHOULD and removing the verification-time read/write denial-of-service bottleneck. The pass adds the deterministic concurrency evidence required by v0.9 section 20.2 (`tests/relay_concurrency.rs`: a writer paused undecided inside the write-critical section, a second winning writer, a concurrent `changes` reader, cancellation without a permanently blocking hole, SQLite rollback of an allocated number, and a reader completing while another publication is paused unlocked mid-preparation). Sections 3–8, all record and envelope bytes, and Appendix B.2–B.12 are unchanged by v0.9, so the reviewed v0.8.1 core, clean-room model, 218/218 differential result, and 53 confirmed fixtures remain the applicable core evidence. The crate implements the complete Followee v1 protocol core against specification v0.9 — strict deterministic CBOR with the layered well-formedness/basic-validity/deterministic-profile classification, the exact COSE Sign1 profile, `did:flw` parsing and derivation, Authority Descriptors and revocation commitments, Contact Document and record schemas, signing, the sole strict-Ed25519 verification entry point with a mechanical direct-call restriction, full-record verification, deterministic ordering, and sticky root revocation — plus the Milestone 2 `followee` command-line client (identity creation with separately stored owner-only seed files, root and RootRevoked record signing, verification and inspection that never present unverified claims as verified, deterministic explicit-target candidate selection, friendly complete-document JSON authoring, and safe local test-key storage that refuses unsafe permissions and every published Appendix B test seed) and, as of Milestone 3, a single bounded relay: a narrow storage contract with observationally identical memory and SQLite backends, the atomic section 13.1 ingress algorithm with exact section 13.2 update-number semantics, opaque generation-committed cursors with the exact two-field `ResetRequired` encoding, and the five mandatory HTTP/CBOR operations (`v1/info`, `v1/resolve`, `v1/directory`, `v1/publish`, `v1/changes`) with the specified media types, statuses, error codes, bounds, and CORS behaviour. The complete Appendix B.2–B.10 and B.12 conformance suite reproduces every published byte sequence exactly, and the relay serves the applicable B.11 wrapper vectors byte-for-byte over real HTTP (B.11.1 outer-fault `400`, B.11.4 duplicate-DID cardinality, B.11.6 aligned per-DID error). Milestone 1.5 differential evidence is complete (`followee-conformance` `v0.8.1-differential-final`, 218/218 agreement; all 53 provisional fixtures independently confirmed) and remains applicable at v0.9, whose amendment leaves the core-model scope unchanged. All recorded specification questions in [`SPEC-QUESTIONS.md`](SPEC-QUESTIONS.md) are resolved. No resolver, synchronization-receiver, or WebFinger code exists yet; Milestone 4 has not begun.

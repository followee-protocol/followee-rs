...
**Milestone 6 (external interoperability) is in progress as the maintained v0.9.2 Rust participant pass for Campaign 2.** Against the pinned specification v0.9.2, this pass: enforces the normative section 12.5 publish-response status-dependent field rules through the one production client decoder (`relay::wire::parse_publish_response`, behind `RelayClient::publish`), while the relay keeps its deterministic no-code status-`1` encoding — a conforming form the campaign records as permitted diagnostic variation, never normalized; closes Campaign 1 finding I1 with the narrow production accessor `FolloweeDid::multihash_bytes()` returning the exact already-validated 34-byte multihash; closes finding I2 by implementing authoring revision 2's exact `verifyRecord` accepted result — the closed eight-member `record.descriptor` projection, the authority-dependent `record.revocationKey`, the exact structured Contact Document with two distinct extension maps, and lossless absent-versus-present-empty output backed by participant-designed direct wire fixtures for the present-empty collections authoring cannot construct (`interop` module, `followee interop`); adds the neutral interface engine covering all nine INTERFACE.md operations through production entry points; adds the mandatory hostile-response rejections (info without protocol version `1`, info without suite `-19`, directory with duplicate indices — each a complete `schemaViolation` rejection with no partial state), the retained-premature-tuple regression evidence, the recorded 65,536-byte HTTP publish request-entity cap with the exactly-16,385-byte fault-isolated `recordTooLarge` case, and naturally obtained foreign-generation cursor evidence; and produces the complete participant-owned v0.9.2 outputs — published-vector results, recipe-constructed negative envelopes, Appendix B.11 wire reproductions, and the blind-challenge rerun — deterministically regenerable and preserved with hashes in [`interop/v0.9.2/`](interop/v0.9.2/). This is a **maintained participant**, not an independent implementation, and it is **not yet interoperable**: the interoperability claim requires the complete pinned Campaign 2 defined by IMPLEMENTATION.md section 13 Milestone 6.

Milestone 5 (handles and public demonstration) is reviewed at `milestone-5-v0.9.1-reviewed`, and its public demonstration is live: the Railway deployment of `demo/public-authority/` passed the complete external probe set on 2026-08-13 at `https://handle-authority-production.up.railway.app/` (evidence in [`demo/public-authority/README.md`](demo/public-authority/README.md)). Milestone 4 remains reviewed at `milestone-4-v0.9-reviewed`. Milestone 4 adds, against the same pinned v0.9 specification: the strict bounded production HTTP/CBOR relay client (`relay::client`) — one client path shared by synchronization, the direct relay commands, and the resolver, with exact paths/methods/media types, deterministic-CBOR-first outer-response validation, byte-string opacity, injected clocks, explicit shared operation budgets, an injectable transport, and a default-public network policy (HTTPS-only, credential-free, no private/loopback/link-local or otherwise sensitive destinations, validated redirects, resolution-pinned connections) with an explicit loopback development policy for tests and the demonstration; the current-state synchronization receiver (`relay::sync`) that consumes a peer's `v1/changes` feed through the ordinary two-phase ingress (no second verifier, ordering, or update-number path — `commit_current` remains the sole assigner), persists peer identity and the exact opaque cursor in both storage backends keyed by the peer's stable relay instance identifier, handles `ResetRequired` by discarding only the cursor, rejects over-`itemLimit` responses completely, and persists the cursor only after the response's accepted entries are durably processed; the section 14 multi-relay resolver (`resolver`) with one aggregate budget (deadline, bytes, requests, visited relays, reference depth) shared across the entire traversal, deterministic FIFO scheduling, local verification of every Full candidate for the explicit requested DID, continuation past Absent/Error/rejected outer responses, cycle detection, sticky RootRevoked retention, per-DID state isolation, and lazy path compression that stores only routing state after a verified traversal; the minimal non-interactive `followee relay publish`, `relay resolve`, `relay changes`, `relay sync`, and multi-relay `followee resolve` commands (JSON output, stable symbols, protocol-versus-infrastructure failure distinction, no protocol decisions in handlers); the Appendix B.11.2/B.11.3/B.11.5/B.11.7 behavioural gates executed through the production client and receiver paths on both backends (all existing B.11 server-side byte-exact tests retained); and the deterministic three-relay shell demonstration (`demo/three_relay_demo.sh`, run by `tests/three_relay_demo.rs`) driving three real `followee relay serve` processes on isolated SQLite databases through the production binary surfaces. Everything the previous milestones delivered is unchanged: the complete v0.9 protocol core, the Milestone 2 authoring CLI, the Milestone 3 relay with its v0.9 concurrent-ingress visibility evidence, the byte-exact Appendix B.2–B.12 conformance suite, and the Milestone 1.5 clean-room differential evidence (218/218; 53 confirmed fixtures) all remain applicable. All recorded specification questions in [`SPEC-QUESTIONS.md`](SPEC-QUESTIONS.md) are resolved; SQ-20 (raised during Milestone 5) was resolved by the specification v0.9.1 amendment, and the production migration classifier implements it exactly: a completed check whose claimant or counterpart winning admissible record is stale is Checked but unverified (`claimantStale`/`counterpartStale`), never Not checked, and never presented.

Milestone 5 adds, against the pinned v0.9.1 specification: the production WebFinger client (`webfinger`) reusing the Milestone 4 bounded HTTP machinery (network policy, SSRF-safe transport, bounded redirects, media types, shared budget meters, injected clocks) with a strict bounded duplicate-rejecting JRD/JSON parser, section 10.1 handle grammar with IDNA2008 lowercase-ASCII domain canonicalization, exact-subject and exactly-one-Followee-link discovery, section 10.3 bootstrap in which every candidate is opaque bytes until complete local verification and deterministic selection through the production core, and section 10.4 inverse handle verification (a signed `alsoKnownAs` value is never called verified without the inverse mapping to the same DID); the three section 14.2 migration presentation states in the resolver (`resolver::check_migration`) with the exact v0.9.1 classification (Verified only for fresh reciprocal pairs; completed stale or non-reciprocal checks are Checked but unverified; Not checked only for incomplete checks), hop bounds, and one shared operation scope across the primary resolution and every migration hop, with no automatic re-following; the non-interactive `followee handle resolve`, `handle verify`, and `handle serve` commands plus `followee resolve --check-migration` (JSON output, stable symbols, production components only, no secret material); the minimal demonstration handle authority (`webfinger::authority`) over a bounded, deterministic, reviewable configuration that can never assign ASCII-case variants of one local to different DIDs, exercised by real-socket black-box tests; and the deployment-ready public-demonstration artifact ([`demo/public-authority/`](demo/public-authority/)) — the canonical reproducible Railway container path (pinned-toolchain multi-stage build, `PORT` binding, explicit operator-configured public base URI) beside the retained provider-neutral VPS path (Caddy/nginx TLS termination, hardened systemd unit), with exact operator instructions and the public probes for HTTPS on a provider-assigned domain — whose example configuration and container entrypoint are themselves served and probed by the test suite.
...
The Rust brief currently targets protocol repository commit `ac5a794f2fdadc13cddf5367fa3e047617e3e950` (specification v0.9.2, revision commit `f1d19fec0dba455d90d473bfad625d1c288e0c15`). When a normative ambiguity is resolved, the brief must be re-pinned and the complete conformance suite rerun.

...
**Milestone 2 (authoring and inspection CLI) delivered, under review.** The crate implements the complete Followee v1 protocol core against specification v0.8.1 — strict deterministic CBOR with the layered well-formedness/basic-validity/deterministic-profile classification, the exact COSE Sign1 profile, `did:flw` parsing and derivation, Authority Descriptors and revocation commitments, Contact Document and record schemas, signing, the sole strict-Ed25519 verification entry point with a mechanical direct-call restriction, full-record verification, deterministic ordering, and sticky root revocation — and, as of Milestone 2, the `followee` command-line client: identity creation with separately stored owner-only seed files, root and RootRevoked record signing, verification and inspection that never present unverified claims as verified, deterministic explicit-target candidate selection, friendly complete-document JSON authoring, and safe local test-key storage that refuses unsafe permissions and every published Appendix B test seed. The complete Appendix B.2–B.10 and B.12 conformance suite reproduces every published byte sequence exactly and rejects every required mutation with fault-isolated fixtures; the B.11 relay-wrapper bytes and digests are reproduced byte-for-byte ahead of their Milestone 3/4 behavioural gates. Milestone 1.5 differential evidence is complete (`followee-conformance` `v0.8.1-differential-final`, 218/218 agreement; all 53 provisional fixtures independently confirmed). All recorded specification questions in [`SPEC-QUESTIONS.md`](SPEC-QUESTIONS.md) are resolved. No relay, resolver, HTTP, or storage code exists on this branch yet; Milestone 3 (single relay) is next.

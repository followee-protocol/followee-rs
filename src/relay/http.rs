...
pub async fn serve(relay: Arc<Relay>, listener: tokio::net::TcpListener) -> std::io::Result<()> {
    serve_with_shutdown(relay, listener, std::future::pending()).await
}

/// [`serve`], stopping cleanly once `shutdown` resolves: in-flight requests
/// complete and the listener closes. The same development-mode loopback
/// guard applies.
///
/// # Errors
///
